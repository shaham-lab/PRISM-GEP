"""Build the 9-dataset cell-ordering table (CSV).
...
Output : results/tables/tab_traj_cell_9ds.csv
         One row per method: the nine per-dataset |rho| values (3 decimals), then
         mean, median and mean_rank (2 decimals) over the nine datasets.
"""
import argparse
...
# Default writes to the release table directory, results/tables/, which tables_dir()
# creates on demand. --out overrides it. The parent is created either way rather than
# failing late, after the numbers have already been computed.
DEFAULT_OUT = tables_dir() / "tab_traj_cell_9ds.csv"

# (key, printed name) in the display order used by the gene-trajectory table.
# Hemogenic and Endoderm have two label ranks only, so those columns cannot separate methods.
NINE = [
    ("pancreas", "Pancreas"),
    ("gastrulation", "Gastrulation"),
    ("gastrulation_erythroid", "Gast. Eryth."),
    ("hemogenic_endothelium", "Hemogenic"),
...
def main():
    ...
    for col, name in METHODS:
        row = {"method": name}
        for k, _ in NINE:
            row[k] = round(float(m.loc[k, col]), 3)
        row["mean"] = round(float(means[col]), 3)
        row["median"] = round(float(meds[col]), 3)
        row["mean_rank"] = round(float(mranks[col]), 2)
        rows.append(row)
