#!/usr/bin/env python
r"""Emit the aggregate robustness table over ALL FIFTEEN datasets (CSV).
...
Out: results/tables/tab_robustness_agg_15ds_vs_specialized.csv
     One row per method, sorted by mean rank: mean_rank, ci_lo, ci_hi (bootstrap over
     datasets, 2 decimals), geom_composite, worst_axis (3 decimals), n_last_place,
     n_rankable, n_datasets. The PRISM-vs-baseline Welch tally is printed.
"""
import os
...
# NOTE: deliberately NOT "robustness_aggregates_15ds.csv". build_unified_table1.py writes that
# filename (under results/) with its own SIX-method convention (MALLET included), and running
# it silently overwrote this table once already. The distinct name keeps the two from colliding.
OUT = os.path.join(str(tables_dir()), "tab_robustness_agg_15ds_vs_specialized.csv")
...
DISP = {"PRISM": "PRISM-GEP", "MALLET": "MALLET", "NMF": "NMF",
...
# 15 datasets, n15 rankable entries, PRISM-GEP vs the four specialized methods (MALLET
# excluded, as in the main-text robustness figure). Mean rank: lower is better; geometric
# composite and worst-axis: higher is better.
rows = []
for m in order:
    d = r15[m]
    rows.append(dict(method=DISP[m], mean_rank=round(d["mr"], 2), ci_lo=round(d["lo"], 2),
                     ci_hi=round(d["hi"], 2), geom_composite=round(d["geom"], 3),
                     worst_axis=round(d["worst"], 3), n_last_place=int(d["last"]),
                     n_rankable=n15, n_datasets=len(ALL15)))
