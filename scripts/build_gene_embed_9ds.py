...
Outputs:
  outputs/gene_embedding_ablation/aggregate_metrics_9ds.csv   (long: dataset, method, ...)
  results/tables/tab_gene_embed_9ds.csv                       (the table: one row per method,
      per-dataset |rho| to 3 decimals, then mean / median / mean_rank over all nine datasets
      and the same three with Gastrulation E7.5 excluded; n_datasets and n_rankable record
      how many datasets each aggregate is taken over)
...
TAB_OUT = tables_dir() / "tab_gene_embed_9ds.csv"

# Column order and the short labels used by the sibling trajectory tables, so the
...
# `random` is a chance floor, not a competing representation, so it is excluded from the
# best-in-group bookkeeping below (it would otherwise take Dentate Gyrus and E7.5, where it
# happens to beat every method). Its values are still reported, so the reader can see it.
NOT_BOLDED = {"random"}
...
GROUP_INT = [
    ...
    ("scgpt", "scGPT static"),
    ("scgpt_contextual", "scGPT contextual"),
]
...
def fmt(v: float):
    """Three decimals as a plain number; undefined cells stay blank (None)."""
    if v is None or (isinstance(v, float) and np.isnan(v)):
        return None
    return round(float(v), 3)


def emit_table(long: pd.DataFrame) -> pd.DataFrame:
    ...
    for group, header in ((GROUP_FULL, "Full marker set"),
        ...
        for m, label in group:
            ...
            row["mean"] = fmt(mu)
            row["median"] = fmt(med)
            row["mean_rank"] = None if np.isnan(mr) else round(float(mr), 2)
            row["mean_excl_e75"] = fmt(mu_r)
            row["median_excl_e75"] = fmt(med_r)
            row["mean_rank_excl_e75"] = None if np.isnan(mr_r) else round(float(mr_r), 2)
            # n_datasets: how many datasets the mean/median is over (fewer than nine when a
            # method is undefined somewhere); n_rankable: datasets where every member of
            # the group is defined, which is what the mean rank is over.
            row["n_datasets"] = int(len(ok))
            row["n_rankable"] = int(len(rankable))
            rows.append(row)
...
def main() -> int:
    ...
    ap.add_argument("--out-table", default=str(TAB_OUT))
    ...
    tab = emit_table(long)
    out_tab = Path(args.out_table)
    out_tab.parent.mkdir(parents=True, exist_ok=True)
    tab.to_csv(out_tab, index=False)
    print(f"WROTE {out_tab}")

    # Marker counts per group, which is the vocabulary-coverage story (scGPT drops 4 of 8
    # markers on Paul15). Also in aggregate_metrics_9ds.csv (column n_genes).
    n_full = [piv_n(long, "prism_full", ds, "n_genes") for ds, _ in DATASETS]
    n_int = [piv_n(long, "prism_int", ds, "n_genes") for ds, _ in DATASETS]
    print("marker counts: full  = " + "/".join(map(str, n_full)))
    print("marker counts: inter = " + "/".join(map(str, n_int)))
